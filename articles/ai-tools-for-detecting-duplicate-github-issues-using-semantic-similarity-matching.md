...
For example, the phrases "app freezes when clicking the button" and "interface becomes unresponsive after user interaction with submit control" convey the same core problem despite sharing no common keywords. A semantic similarity model trained on codebases and technical language can recognize that both describe a UI freezing issue.
