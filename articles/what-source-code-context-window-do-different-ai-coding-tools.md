...
When using Claude Code with cloud models, your code context processes through Anthropic's API. The company has implemented robust privacy commitments, stating that API inputs are not used for training without explicit opt-in. Enterprise customers have additional data processing guarantees.
