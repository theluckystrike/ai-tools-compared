...
This approach lets each skill start streaming its output quickly, giving users immediate feedback that something is happening. The **tdd** skill exemplifies this pattern by separating test analysis from test generation, allowing each phase to stream independently.
