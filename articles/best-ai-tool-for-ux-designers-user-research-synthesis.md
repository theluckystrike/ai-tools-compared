...
Imagine you have conducted twelve user interviews about a new feature. Each interview lasts forty-five minutes, producing roughly twelve pages of transcript when transcribed. Manually coding these transcripts, identifying problems, motivations, and behaviors, could take a designer eight to twelve hours.
...
- Optionally: analyze support tickets or user feedback
- Optionally: watch usability test recordings

Prepare your data for AI analysis:
- Transcribe interviews (use Rev.com, Descript, or similar if not AI)
...
5. USER SEGMENTS: Are there different user types with different needs?
   - Segment 1: [characteristics, needs, problems]
   - Segment 2: [characteristics, needs, problems]
...
- Are frequencies correct (you watched the data)?
- Are any important themes missing?
- Do the suggested user segments make sense?
...
```
In the analysis above, you identified [theme] as a top problem.
Based on these interview excerpts, what are the root causes of this problem?
...
would address the top 3 problems?
...
- Confirms this matches your impressions
- Adds context: "The enterprise users also mentioned budget tracking, which the AI missed"
- Notes: "Freelancers care about invoicing integration, which came up in 2 interviews"

Your refinement question:
"The AI identified time tracking as the #1 problem.
What specifically makes it confusing? Is it UX issues,
lack of features, or workflow mismatch?"

AI's deeper analysis:
"Based on specific quotes, the confusion stems from:
- Unclear start/stop affordances (users don't notice timer controls)
- Difficulty categorizing time after the fact (no good way to tag/organize)
- No mobile option (users want to log time from phone)
