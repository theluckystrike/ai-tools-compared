...
1. Prioritizing by context: Critical vulnerabilities in exposed services rank higher than low-severity issues in unexposed dependencies
...
- Scan early and often: Integrate scanning into local development workflows using `trivy fs .` before committing
...
Start with Trivy—it is fully open-source, has no per-image scan limits, and integrates into GitHub Actions with a single `uses:` line. When you need automated fix PRs and reachability analysis, add Snyk Container on its free tier, which covers up to 200 open-source tests per month.

**Q: How do I handle vulnerabilities in base images that have no fix available?**
Use `trivy image --ignore-unfixed` to suppress unfixed CVEs in reports, reducing noise without hiding actionable findings. For tracking purposes, use Snyk's dashboard to watch unfixed base image CVEs separately from application-layer findings. Consider switching to a distroless base image to eliminate entire categories of OS-level vulnerabilities.

**Q: What is the difference between image scanning and runtime protection?**
Image scanning (Trivy, Snyk, Scout) catches known CVEs and misconfigurations before deployment. Runtime protection (Falcon, Sysdig) detects behavioral anomalies in running containers—privilege escalation, unexpected outbound connections, binary execution not present in the original image. Both layers are needed for complete coverage.

**Q: Can AI scanning tools detect secrets accidentally baked into images?**
Yes. Trivy includes secret detection using entropy analysis and pattern matching. Run `trivy image --scanners vuln,secret myapp:latest` to check both CVEs and secrets simultaneously. Snyk also detects secrets during container scanning. For prevention, use `docker buildx build --secret` to mount secrets at build time without persisting them in image layers.
