...
The most common cause of API key errors is incorrect or missing environment variables. Claude Code reads API keys from environment variables, and the naming must match exactly what the service expects.
...
Some Claude skills include their own API key requirements. If a particular skill is throwing the invalid API key error, check its documentation or the skill file itself.
